...
# ─── Copy HTML template to script dir ──────────────────────────────────────
def _copy_html_template():
    """Copy pdb_index.html from templates/ to script dir if not present."""
    script_dir = config.get_script_dir()
    dest = script_dir / "pdb_index.html"
    if dest.exists():
        return
    # Find template in package
    pkg_root = Path(__file__).parent
    src = pkg_root / "templates" / "pdb_index.html"
    if src.exists():
        import shutil
        shutil.copy2(src, dest)
        app.logger.info(f"Copied HTML template to {dest}")
